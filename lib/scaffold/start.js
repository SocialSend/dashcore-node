...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and socialsendcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/socialsendcore-node/blob/bitcoind/docs/upgrade.md \n');
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/socialsendd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed socialsendcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './socialsendcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in socialsendcore-node services directory
